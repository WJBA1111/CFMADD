...
// 现代门红石支持
public class BlockModernSlidingDoorRedStoneSupport extends BlockModernSlidingDoor {
    ...
    public BlockModernSlidingDoorRedStoneSupport() {
        super();
    ...
    protected void setPowered(World world, BlockPos pos, IBlockState state, boolean powered) {
        TileEntity tileEntity = world.getTileEntity(pos);
        if (tileEntity instanceof TileEntityModernSlidingDoor) {
            TileEntityModernSlidingDoor slidingDoor = (TileEntityModernSlidingDoor)tileEntity;
            ...
            slidingDoor.sync();
            if (powered) {
                world.playSound((EntityPlayer)null, pos, FurnitureSounds.sliding_door_open, SoundCategory.BLOCKS, 1.0F, 1.0F);
            } else {
                world.playSound((EntityPlayer)null, pos, FurnitureSounds.sliding_door_close, SoundCategory.BLOCKS, 1.0F, 1.0F);
            }
            ...
            EnumFacing slidingFacing = ((EnumFacing)state.getValue(FACING)).rotateYCCW();
            BlockPos otherDoor = pos.offset(slidingFacing.getOpposite());
            IBlockState state1 = world.getBlockState(otherDoor);
            if (state1.getBlock() instanceof BlockModernSlidingDoorRedStoneSupport) {
                ((BlockModernSlidingDoorRedStoneSupport)state1.getBlock()).setPowered(world, otherDoor, state1, powered);
            }
        }
    ...
    public AxisAlignedBB getBoundingBox(IBlockState state, IBlockAccess source, BlockPos pos) {
        ...
        if ((Boolean)state.getValue(TOP)) {
            return open ? COLLISION_BOX_TOP_OPENED[facing.getHorizontalIndex()] : COLLISION_BOX_TOP[facing.getHorizontalIndex()];
        } else {
            return open ? COLLISION_BOX_BOTTOM_OPENED[facing.getHorizontalIndex()] : COLLISION_BOX_BOTTOM[facing.getHorizontalIndex()];
        }
    }
    ...
    public void neighborChanged(IBlockState state, World worldIn, BlockPos pos, Block blockIn, BlockPos fromPos) {
        if (!worldIn.isRemote && blockIn instanceof BlockModernSlidingDoorRedStoneSupport) {
