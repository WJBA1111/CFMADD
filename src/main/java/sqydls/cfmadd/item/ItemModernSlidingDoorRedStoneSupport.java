...
public class ItemModernSlidingDoorRedStoneSupport extends ItemModernSlidingDoor {


    public ItemModernSlidingDoorRedStoneSupport(Block block) {
        super(block);
    }
    ...
    public EnumActionResult onItemUse(EntityPlayer player, World worldIn, BlockPos pos, EnumHand hand, EnumFacing facing, float hitX, float hitY, float hitZ) {
        if (facing != EnumFacing.UP) {
            ...
            if (player.canPlayerEdit(pos, facing, stack) && this.block.canPlaceBlockAt(worldIn, pos)) {
                ...
                this.placeSlidingDoor(worldIn, pos, playerFacing, this.block, side);
    ...
    private void placeSlidingDoor(World world, BlockPos pos, EnumFacing facing, Block block, boolean rightSide) {
        ...
        world.setBlockState(pos.up(), block.getDefaultState().withProperty(BlockFurniture.FACING, doorFacing).withProperty(BlockModernSlidingDoorRedStoneSupport.TOP, true), 3);
    }
